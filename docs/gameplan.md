...
| 15 | **Reader can change the inputs (artists, genres, travel range).** | The product could be read but not played with. Scoring never needed the API key — only fetching did — so the candidate pool ships to the browser and `lib/rank.mjs`, the same module the build runs, re-ranks locally on every change. No server, no key, no model in the request path. Genre is a third taste tier at 12, labelled so it is never confused with a named artist. |
